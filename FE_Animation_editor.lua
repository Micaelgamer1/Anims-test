...
local Animations
if isfile("animator.json") then
    local data = readfile("animator.json")
    Animations = HttpService:JSONDecode(data)
    Notify("Loading..", "Animations from animator.json", 3)
else
    writefile("animator.json", HttpService:JSONEncode(OriginalAnimations))
    Animations = OriginalAnimations
    Notify("Saved", "Original animations to animator.json", 3)
end
...
local Presets
if isfile("presets.json") then
    local data = readfile("presets.json")
    Presets = HttpService:JSONDecode(data)
else
    Presets = {}
    writefile("presets.json", HttpService:JSONEncode(Presets))
end
...
addTabButton.Size = UDim2.new(0.333, 0, 1, 0)
...
removeTabButton.Size = UDim2.new(0.333, 0, 1, 0)
removeTabButton.Position = UDim2.new(0.333, 0, 0, 0)
removeTabButton.Parent = tabFrame

local presetTabButton = Instance.new("TextButton")
presetTabButton.Text = "Presets"
presetTabButton.Font = Enum.Font.SourceSansBold
presetTabButton.TextScaled = true
presetTabButton.TextColor3 = Color3.fromRGB(255, 255, 255)
presetTabButton.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
presetTabButton.Size = UDim2.new(0.334, 0, 1, 0)
presetTabButton.Position = UDim2.new(0.666, 0, 0, 0)
presetTabButton.Parent = tabFrame
...
local presetContent = Instance.new("Frame")
presetContent.Size = UDim2.new(1, 0, 1, 0)
presetContent.BackgroundTransparency = 1
presetContent.Visible = false
presetContent.Parent = contentFrame

local presetNameBox = Instance.new("TextBox")
presetNameBox.Name = "PresetNameBox"
presetNameBox.Text = ""
presetNameBox.PlaceholderText = "Nome do preset..."
presetNameBox.Font = Enum.Font.SourceSans
presetNameBox.TextScaled = true
presetNameBox.TextColor3 = Color3.fromRGB(200, 200, 200)
presetNameBox.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
presetNameBox.BorderSizePixel = 0
presetNameBox.Size = UDim2.new(0.9, 0, 0.1, 0)
presetNameBox.Position = UDim2.new(0.05, 0, 0, 0)
presetNameBox.Parent = presetContent

local savePresetButton = Instance.new("TextButton")
savePresetButton.Text = "Salvar Preset Atual"
savePresetButton.Font = Enum.Font.SourceSansBold
savePresetButton.TextScaled = true
savePresetButton.TextColor3 = Color3.fromRGB(255, 255, 255)
savePresetButton.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
savePresetButton.Size = UDim2.new(0.9, 0, 0.1, 0)
savePresetButton.Position = UDim2.new(0.05, 0, 0.11, 0)
savePresetButton.Parent = presetContent

local presetSearchBox = Instance.new("TextBox")
presetSearchBox.Name = "PresetSearch"
presetSearchBox.Text = ""
presetSearchBox.PlaceholderText = "Search presets..."
presetSearchBox.Font = Enum.Font.SourceSans
presetSearchBox.TextScaled = true
presetSearchBox.TextColor3 = Color3.fromRGB(200, 200, 200)
presetSearchBox.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
presetSearchBox.BorderSizePixel = 0
presetSearchBox.Size = UDim2.new(0.9, 0, 0.1, 0)
presetSearchBox.Position = UDim2.new(0.05, 0, 0.23, 0)
presetSearchBox.Parent = presetContent

local presetScrollFrame = Instance.new("ScrollingFrame")
presetScrollFrame.Size = UDim2.new(0.9, 0, 0.55, 0)
presetScrollFrame.Position = UDim2.new(0.05, 0, 0.35, 0)
presetScrollFrame.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
presetScrollFrame.BorderSizePixel = 0
presetScrollFrame.ScrollBarThickness = 5
presetScrollFrame.ScrollingDirection = Enum.ScrollingDirection.Y
presetScrollFrame.CanvasSize = UDim2.new(0, 0, 0, 0)
presetScrollFrame.Parent = presetContent

local presetButtons = {}
local SELECTEDPRESET = nil
local SELECTEDPRESETBTN = nil

local function RefreshPresetList(filter)
    filter = filter or ""
    ...
    for presetName, _ in pairs(Presets) do
        if filter == "" or presetName:lower():find(filter:lower()) then
            local button = Instance.new("TextButton")
            ...
            button.Parent = presetScrollFrame

            button.MouseButton1Click:Connect(function()
                if SELECTEDPRESET == presetName then
                    SELECTEDPRESET = nil
                    if SELECTEDPRESETBTN then
                        SELECTEDPRESETBTN.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
    ...
    presetScrollFrame.CanvasSize = UDim2.new(0, 0, 0, yPos)
end

presetSearchBox:GetPropertyChangedSignal("Text"):Connect(function()
    RefreshPresetList(presetSearchBox.Text)
end)

local applyPresetButton = Instance.new("TextButton")
applyPresetButton.Text = "Aplicar Preset"
applyPresetButton.Font = Enum.Font.SourceSansBold
applyPresetButton.TextScaled = true
applyPresetButton.TextColor3 = Color3.fromRGB(255, 255, 255)
applyPresetButton.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
applyPresetButton.Size = UDim2.new(0.44, 0, 0.1, 0)
applyPresetButton.Position = UDim2.new(0.05, 0, 0.9, 0)
applyPresetButton.Parent = presetContent

local deletePresetButton = Instance.new("TextButton")
deletePresetButton.Text = "Deletar Preset"
deletePresetButton.Font = Enum.Font.SourceSansBold
deletePresetButton.TextScaled = true
deletePresetButton.TextColor3 = Color3.fromRGB(255, 255, 255)
deletePresetButton.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
deletePresetButton.Size = UDim2.new(0.44, 0, 0.1, 0)
deletePresetButton.Position = UDim2.new(0.51, 0, 0.9, 0)
deletePresetButton.Parent = presetContent

applyPresetButton.MouseButton1Click:Connect(function()
    if not SELECTEDPRESET then
        Notify("Erro", "Selecione um preset para aplicar", 3)
        return
    end
    local presetData = Presets[SELECTEDPRESET]
    if not presetData then
        Notify("Erro", "Preset não encontrado", 3)
        return
    end
    local typeOrder = {"Idle", "Walk", "Run", "Jump", "Fall", "Swim", "SwimIdle", "Climb"}
    for _, animType in ipairs(typeOrder) do
        if presetData[animType] then
            setAnimation(animType, presetData[animType])
        end
    end
    Notify("Aplicado", SELECTEDPRESET, 2)
end)

deletePresetButton.MouseButton1Click:Connect(function()
    if not SELECTEDPRESET then
        Notify("Erro", "Selecione um preset para deletar", 3)
        return
    end
    Presets[SELECTEDPRESET] = nil
    savePresetsToFile()
    Notify("Deletado", SELECTEDPRESET, 2)
    SELECTEDPRESET = nil
    SELECTEDPRESETBTN = nil
    RefreshPresetList(presetSearchBox.Text)
end)
...
savePresetButton.MouseButton1Click:Connect(function()
    local presetName = presetNameBox.Text
    if presetName == "" then
        Notify("Erro", "Digite um nome para o preset", 3)
        return
    end
    if Presets[presetName] then
        Notify("Erro", "Já existe um preset com esse nome", 3)
        return
    end
    ...
    for _, animType in ipairs(typeOrder) do
        if lastAnimations[animType] then
            snapshot[animType] = lastAnimations[animType]
            hasAny = true
        end
    end

    if not hasAny then
        Notify("Erro", "Nenhuma animação ativa para salvar", 3)
        return
    end

    Presets[presetName] = snapshot
    savePresetsToFile()
    Notify("Salvo", "Preset \"" .. presetName .. "\" salvo", 3)
    presetNameBox.Text = ""
    RefreshPresetList(presetSearchBox.Text)
end)
...
presetTabButton.MouseButton1Click:Connect(function()
    addContent.Visible = false
    removeContent.Visible = false
    presetContent.Visible = true
    addTabButton.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
    removeTabButton.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
    presetTabButton.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
...
actionButton.MouseButton1Click:Connect(function()
    ...
    writefile("animator.json", HttpService:JSONEncode(Animations))
...
local function saveLastAnimations(lasyAnimations)
    ...
    pcall(function() writefile("lastAnimations.json", data) end)
end
...
if isfile("animator.json") then
    ...
    Notify("Loaded", "Animations from animator.json", 3)
else
    writefile("animator.json", HttpService:JSONEncode(OriginalAnimations))
...
local function LoadAndUpdateAnimations()
    ...
    if hadChanges then
        ...
        local newFileData = readfile("animator.json")
